...
- `src/i18n/locales/*.json`：首页与工作台演示的全部文案，一种语言一个文件，与应用本体的 `web/src/i18n/locales/` 保持一致。
- `src/i18n/types.ts`：文案结构与语言表；`src/i18n/ui.ts` 把 JSON 按 `Record<Lang, SiteCopy>` 装配，漏字段会在 `npm run check` 报错并指出具体路径。
- `src/layouts/HomePage.astro`：首页结构，按 `lang` 渲染；`src/pages/index.astro` 与 `src/pages/en/index.astro` 只负责传入语言。
- `src/components/Lines.astro`：把文案数组渲染成多行，行间补 `<br>` 与空格。
- `src/components/WorkbenchPreview.astro`：工作台演示。信息架构与用词照搬应用本体，改动前对照 `web/src/i18n/locales/`——官网自造说法会让首屏和用户装到的软件对不上。示例流量一律用 `example.com` 等保留域名。
- `src/styles/home.css`：首页样式与响应式布局。
...
3. 在 `src/i18n/ui.ts` 的 `copy` 表里引入这份 JSON，缺字段此时会报错。

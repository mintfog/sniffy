...
选择对应平台的安装包，点击文件名即可下载：

<Downloads edition="desktop" />

安装包通过 CDN 下载，也可从 [GitHub Releases](https://github.com/mintfog/sniffy/releases) 获取。将鼠标停在文件名上可查看 SHA256 校验值。
...
如果提示应用「已损坏」，请先确认安装包来自本站下载页或本项目的 Releases 页面，且下载完整。确认是隔离属性导致的拦截后，可执行：
...
headless 可执行文件按 `sniffy-<os>-<arch>` 命名，是纯静态的单文件，没有运行时依赖：
...
配置区存放 `config.json`、`rules.json`、日志、用户插件与证书；缓存区存放大体积响应体的落盘副本，以及下载目录不可用时的更新安装包，随时可以删除。完整说明见[配置参考](/docs/configuration/)。

## 升级与卸载

桌面版会在启动后检查更新，此后每天检查一次。有新版本时，点击状态栏提示，在「关于」窗口下载安装包。更新信息来自官网，安装包通过 CDN 下载。

下载完成后，Sniffy 会校验安装包的 SHA256。安装包保存在系统的「下载」目录；该目录不可用时，保存在缓存区的 `updates/` 目录。

- **Windows**：点击「立即安装」并确认系统授权，Sniffy 会关闭，安装完成后自动重新打开。
- **macOS**：点击「打开安装镜像」，退出 Sniffy，再把镜像中的 Sniffy 拖进「应用程序」并替换旧版本。
- **Linux**：点击「打开所在文件夹」，退出 Sniffy，再执行 `sudo apt install ./sniffy-desktop-linux-amd64-installer.deb`。

可以在「设置 - 关于」中关闭自动检查，也可以在「关于」窗口选择「跳过此版本」。跳过后，更新的版本仍会提醒。

无界面版通过日志提示新版本，也可以通过管理 API 查询更新状态。下载后，停止服务、替换可执行文件并重新启动。自建分发渠道可用环境变量 `SNIFFY_UPDATE_FEED` 指定清单地址，多个地址用逗号分隔，只接受 https 地址；清单中的每个产物都须登记 `kind`（`installer` 或 `binary`）、`size` 与 `sha256`，否则整份清单会被拒绝。

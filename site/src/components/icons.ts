...
/**
 * 站点图标沿用应用本体的 lucide（web/ 依赖 lucide-react），
 * 首屏的工作台才会和用户装到的软件长得一样。
 */
const lucideNames = {
  arrow: "arrow-right",
  download: "download",
...
const brandPaths = {
  ...
  windows:
    "M3 5l8-1v7H3zm10-1 8-1v8h-8zM3 13h8v7l-8-1zm10 0h8v8l-8-1z",
  linux:
...
function lucideBody(file: string): string {
  return readFileSync(require.resolve(`lucide-static/icons/${file}.svg`), "utf8")
    .replace(/^[\s\S]*?<svg[^>]*>/, "")
    .replace(/<\/svg>\s*$/, "")
    .trim();
}
